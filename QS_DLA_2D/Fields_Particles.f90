...
module output_var
  ...
  real(RP),allocatable :: den2d(:,:),den_e2d(:,:),jr2d(:,:),jz2d(:,:)
    ...
    integer :: Types
    logical:: Periodic
    real(RP)::a0,Wist0,k0,FocalPlane,xcenter,zcenter,FWHM,centroid,Rise 
...
module TestPart_var
USE global_variables
integer :: NTP_i(5),NTP,Beam_Type(5),polarity(5),N_per_dt,Tp_Dt_per_out,pushtype,Num_of_Beam   !NTP=TP_len*TP_perZ+TP_Rad*TP_perR
real(RP) :: TP_len(5),TP_Rad(5),TP_z0(5),TP_PlateauBe(5),TP_FWHM(5),TP_w0(5),GamaT(5),&
Den_Beam(5),TP_perZcell(5),TP_perRcell(5),XSpread(5),beamchirp(5)
real(RP),allocatable:: Err2d(:,:),Ezz2d(:,:),Bthh2d(:,:),&
Err2dold(:,:),Ezz2dold(:,:),Bthh2dold(:,:)
logical :: TestParts,LaserEZ,Interaction
complex(RP),allocatable:: EL2d(:,:),BL2d(:,:),EL2dold(:,:),BL2dold(:,:),ELZ2dold(:,:),ELZ2d(:,:)
real(RP),allocatable:: XXT(:),ZZT(:),PXT(:),PZT(:),denT(:),denb2d(:,:),jzb2d(:,:)
real(RP),allocatable:: WX_L(:),WZ_L(:),WX_W(:),WZ_W(:)
real(RP),allocatable:: XXT2d(:,:),ZZT2d(:,:),PXT2d(:,:),PZT2d(:,:)
integer(RP),allocatable:: beamidx(:),ifpush(:)
...
USE global_variables
	real(RP),allocatable :: r(:),V(:),force(:),gama(:),mass(:),r0(:)
...
module domain
  ...
  use TestPart_var
    implicit none 
    integer :: &
    ncidf,ncidp,ncidtp,ncids,ztime_dim,ztime_dim2, p_dim, &
    r_dim,dims(2),var_de,var_jr,var_dr,var_deb,var_jzb,&
    var_bt,var_ps,var_jz,var_ez,var_er,&
    var_vr,var_rr,var_fr,var_ri,var_rf,&
    var_ga,var_AR,var_AI,var_BR,var_BI,&
    var_Tx,var_Ty,var_Tz,var_Tpx,var_Tpy,var_Tpz,var_ELR,var_ELI,&
    var_Wxl,var_Wxw,var_Wyw,var_Wzw,var_EZLR,var_EZLI,var_Wzl

    NAMELIST/Lasers/ Laser,a0,Wist0,k0,FocalPlane,xcenter,zcenter,FWHM,Rise,Types,Periodic
    NAMELIST/Simulation/ Rmax,Zmax,Nr,Nz,Part_per_cell,Push_per_cell,Push_Order,dt0,&
    &Tmax,Dt_per_out,ramp_len,rampBegin,Domain_Shift,Restart
    NAMELIST/TestParticles/TestParts,LaserEZ,Interaction,polarity,Num_of_Beam,Den_Beam,Beam_Type,&
    TP_len,TP_Rad,TP_z0,TP_FWHM,TP_w0,beamchirp,TP_PlateauBe,GamaT,XSpread,TP_perZcell,TP_perRcell,N_per_dt,Tp_Dt_per_out,pushtype
