...
!  F,S       jr(1),       Bth(1),         Er(1)  ************************   r=0
    ...
    SUBROUTINE electron_push(r,V,gama,fv,f,Er,Et,Bth,ponderF,Psi,As)
    implicit none
    real(RP) ::delta,Er_p,Bth_p,Et_p,psi_p,As_p,ponder_p
    real(RP) :: r(1:Np),V(1:Np),fv(1:Np),f(1:Np),gama(1:Np)
    real(RP) :: Er(1:Nr+1),Et(1:Nr+1),Bth(1:Nr+1),ponderF(1:Nr+1),Psi(1:Nr+1),As(1:Nr+1)
    integer:: i,n
